...
for i, (task, data) in enumerate(datasets.items()):
    ...
    trainer = EWCTrainer(
        ...
        eval_dataset=X_val,
        ewc=ewc if i > 0 else None  # niente EWC sul primo task
    )
    ...
    # ====== UPDATE FISHER ======
    print(f"Updating Fisher after task {task}...")

    subset_size = min(len(X_train), 500)
    fisher_subset = X_train.select(range(subset_size))

    # remove "text" column if present
    fisher_subset = fisher_subset.remove_columns("text")
    ...
    ewc.update(best_model, fisher_loader)

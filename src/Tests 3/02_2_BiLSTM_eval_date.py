...
RESULTS_DIR_PATH = "results/results_BiLSTM_date"
bool_150 = False
bool_150test = False
...
def prepare_data(datasets, max_words=20000, max_len=300):
    ...
    for date, df in datasets.items():
        ...
        if bool_150: # split dataset in train/val/test (max 150 samples in val and test)
            processed_datasets[date] = split_dataset_tensorflow_150(X, y)
        else: # split dataset in train/val/test
            processed_datasets[date] = split_dataset_tensorflow(X, y)

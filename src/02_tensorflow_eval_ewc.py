...
from utils.utils_continual.utils_ewc import *
...
LAMBDA = 300
FISHER_DECAY = 0.95
RESULTS_DIR_PATH = f"results_ewc/results_{MODEL}_{TASK_TYPE}"
...
# define EWC object
ewc_handler = EWCTensorFlow(
    best_model,
    lambda_ewc=LAMBDA,
    fisher_decay=FISHER_DECAY
)
# define model with EWC
ewc_model = EWCModel(best_model, ewc_handler)
ewc_model.compile(
...
for i, (task, data) in enumerate(datasets.items()):
    ...
    print(f"Updating Fisher for task {task}...")
    subset_idx = np.random.choice(len(X_train), size=min(500, len(X_train)), replace=False)
    ewc_handler.update((X_train[subset_idx], y_train[subset_idx]))

    y_pred = ewc_model.predict(X_test)
    ...
    preds_cumulative = ewc_model.predict(X_cum_test_concat)
    ...
    print("\n--- Evaluation on full test set ---")
    preds_full = ewc_model.predict(X_full_test)
...
path = f"{RESULTS_DIR_PATH}/results_single_{TASK_TYPE}_{LAMBDA}.json"
with open(path, "w") as f:
    json.dump(results_task, f, indent=4)

# save results for Cumulative Test Set
path = f"{RESULTS_DIR_PATH}/results_cumulative_{TASK_TYPE}_{LAMBDA}.json"
with open(path, "w") as f:
    json.dump(results_cumulative, f, indent=4)

# save results for Full Test Set
path = f"{RESULTS_DIR_PATH}/results_full_{TASK_TYPE}_{LAMBDA}.json"
with open(path, "w") as f:
...
path = f"{RESULTS_DIR_PATH}/f1_by_{TASK_TYPE}_{LAMBDA}.png"
plot_f1_by_task(results_task, type=TASK_TYPE.capitalize(), path=path)

path = f"{RESULTS_DIR_PATH}/f1_cumulative_test_{TASK_TYPE}_{LAMBDA}.png"
plot_f1_cumulative_test(results_cumulative, type=TASK_TYPE.capitalize(), path=path)

path = f"{RESULTS_DIR_PATH}/f1_full_test_{TASK_TYPE}_{LAMBDA}.png"

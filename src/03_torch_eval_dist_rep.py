...
MODEL = "RoBERTa" # specify the model type ("BERT", "RoBERTa", "DeBERTa")
TEMPERATURE = 2.0
ALPHA = 0.3
DIM = 50 # number of samples per task to store in replay buffer
RESULTS_DIR_PATH = f"results_dist_rep/results_{MODEL}_{TASK_TYPE}"
...
for i, (task, data) in enumerate(datasets.items()):
    ...
    # Add original training data to replay buffer
    replay_buffer.add(task, train_dataset, y_train)
    print(f"Task '{task}' added to replay buffer.\n")

    # update teacher
    if teacher_model is not None:
        del teacher_model # Elimina il vecchio teacher
        torch.cuda.empty_cache() # Libera memoria
    teacher_model = copy.deepcopy(best_model).to(device)
    teacher_model.eval()
    ...
    for test_task, test_data in datasets.items(): # for each topic/date
        test_task_dataset, y_test_task = test_data["test"]
        preds = trainer.predict(test_task_dataset)
...
path = f"{RESULTS_DIR_PATH}/results_single_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.json"
with open(path, "w") as f:
    json.dump(results_task, f, indent=4)

# save results for Cumulative Test Set
path = f"{RESULTS_DIR_PATH}/results_cumulative_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.json"
with open(path, "w") as f:
    json.dump(results_cumulative, f, indent=4)

# save results for Full Test Set
path = f"{RESULTS_DIR_PATH}/results_full_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.json"
with open(path, "w") as f:
...
path = f"{RESULTS_DIR_PATH}/f1_by_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.png"
plot_f1_by_task(results_task, type=TASK_TYPE.capitalize(), path=path)

path = f"{RESULTS_DIR_PATH}/f1_cumulative_test_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.png"
plot_f1_cumulative_test(results_cumulative, type=TASK_TYPE.capitalize(), path=path)

path = f"{RESULTS_DIR_PATH}/f1_full_test_{TASK_TYPE}_{str(ALPHA).replace('.', '')}_{DIM}.png"

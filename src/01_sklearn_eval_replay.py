...
for i, (task, data) in enumerate(datasets.items()):
    ...
    if X_replay is not None:
        ...
        X_train_combined = pd.concat([X_train, X_replay]).reset_index(drop=True)
        y_train_combined = pd.concat([y_train, y_replay]).reset_index(drop=True)
    else:
        X_train_combined = X_train
        y_train_combined = y_train

    # Vectorization (use vectorized data)
    X_train_vect = best_model.named_steps['vect'].transform(X_train_combined)
    X_val_vect = best_model.named_steps['vect'].transform(X_val)

    if MODEL == "NB":
        # incremental fit (on train+val)
        X_task = pd.concat([X_train_combined, X_val]).reset_index(drop=True)
        y_task = pd.concat([y_train_combined, y_val]).reset_index(drop=True)
        ...
        for epoch in range(n_epochs):
            # Training Epoch
            X_task, y_task = shuffle(X_train_vect, y_train_combined, random_state=None)
            best_model.named_steps['clf'].partial_fit(
                X_task,
                y_task,
                classes=[0,1]
    ...
    # Add original training data (not vectorized) to replay buffer

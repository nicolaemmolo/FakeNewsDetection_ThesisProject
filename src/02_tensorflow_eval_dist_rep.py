...
MODEL = "CNN" # specify the model type ("CNN" or "BiLSTM")
TEMPERATURE = 2.0
ALPHA = 0.5
...
for i, (task, data) in enumerate(datasets.items()):
    ...
    if X_replay is not None:
        ...
        X_train_combined = np.concatenate([X_train, X_replay], axis=0)
        y_train_combined = np.concatenate([y_train, y_replay], axis=0)
    ...
    replay_buffer.add(task, X_train, y_train)

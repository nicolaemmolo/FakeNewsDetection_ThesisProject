...
RESULTS_DIR_PATH = f"results/results_RoBERTa_{TASK_TYPE}"
bool_150 = False
...
def objectiveRoBERTa(trial, X_train, y_train, X_val, y_val):
    """
    Optuna objective function for hyperparameter optimization of RoBERTa model.
    ...
    Returns:
        f1: Validation F1 score to be maximized.
    """
    ...
    model, train_args = build_model(best_params["learning_rate"], best_params["weight_decay"], epochs=1)
    trainer = Trainer(
        model=model,
        args=train_args,
...
objective_with_data = partial(objectiveRoBERTa,
                               X_train=X_train,
                               y_train=y_train,
                               X_val=X_val,
                               y_val=y_val)
...
study.optimize(objective_with_data, n_trials=1) # 30 trials; increase for better results
